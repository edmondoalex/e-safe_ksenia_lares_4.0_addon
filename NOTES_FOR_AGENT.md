...
- Partitions: aggiunto sensore "Stato sensori in allarme" per partizione (lista zone in allarme) che si azzera al disinserimento (v5.2.19).
  - File: `app/main.py`, `config.yaml`
  - Versione: `config.yaml` -> 5.2.19 (alarm zones per partition)

- Partitions: robustezza calcolo sensori in allarme (trigger anche su update dict, parsing PRT tipo lista/bitmask, fallback) (v5.2.20).
  - File: `app/main.py`, `config.yaml`
...
- Systems/Partitions: log STATUS_SYSTEM quando verbose + fallback ALARM_MEM per ricavare zone in allarme (v5.2.22).
...
- Partitions: mapping zone->partition via PRT migliorato (euristica direct/compact + compatibile con ID partizioni non sequenziali) (v5.2.24).
  - File: `app/main.py`, `config.yaml`
...
- Partitions: aggiunto binary_sensor armato/disarmato per partizione (ignora ritardo/immediato) (v5.2.28).
  - File: `app/main.py`, `config.yaml`
...
- Outputs: fix HA unknown (payload include STA a root + value_template outputs usa .get per realtime.STA) (v5.2.31).
  - File: `app/main.py`, `config.yaml`
...
## 5.2.45
- UI: pagina Programmatori orari rimossi Cerca, Ultimo aggiornamento e filtro Solo attivi.
- File toccati: app/debug_server.py, config.yaml, NOTES_FOR_AGENT.md
- Versione: 5.2.45 (semplifica UI programmatori)
